...
docker run --rm -u root -ti -v $(pwd)/mf6.0.2:/tmp/mf6.0.2 -v $(pwd)/prepare-test-env.sh:/tmp/prepare-test-env.sh --entrypoint=/bin/bash $IMAGE \
-c "apt-get update && apt-get install -y git; cd /tmp; ./prepare-test-env.sh; cd /tmp/mf6.0.2/examples; ./modflow-tests.sh"
